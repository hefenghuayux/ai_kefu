...
async def validate_cypher_query_with_llm(
    ...
    if llm_output.filters:
        ...
        for filter in llm_output.filters:
            props = node_props.get(filter.node_label, [])
            matching_props = [
                prop
                for prop in props
...
def extract_cypher_parameters(cypher_statement: str, task: str) -> Dict[str, Any]:
    """Extract parameters for common Text2Cypher placeholders generated by the LLM."""
    parameter_names = set(re.findall(r"\$(\w+)", cypher_statement))
    ...
    for parameter_name in parameter_names:
        ...
        if normalized_name in {"order_id", "orderid"}:
            order_match = re.search(r"订单\s*([0-9]+)", task) or re.search(r"\b([0-9]+)\b", task)
            if order_match:
                parameters[parameter_name] = order_match.group(1)
        elif normalized_name == "supplier_name":
            supplier_match = re.search(r"([\u4e00-\u9fa5A-Za-z0-9]+(?:智能生活|智能家庭))", task)
            if supplier_match:
                parameters[parameter_name] = supplier_match.group(1)
        elif normalized_name == "product_name":
            product_match = re.search(
                r"(谷歌\s*智能门铃\s*Basic|华为\s*智能门铃\s*Plus|格力\s*智能空调\s*Standard|苹果\s*智能电视\s*Max|苹果\s*智能插座\s*Elite|博世\s*智能冰箱\s*Lite)",
                task,
            )
            if product_match:
                parameters[parameter_name] = product_match.group(1)
        elif normalized_name == "customer_name":
            customer_match = re.search(r"([\u4e00-\u9fa5]+有限公司[\u4e00-\u9fa5]*)", task)
            if customer_match:
                parameters[parameter_name] = customer_match.group(1)
        elif normalized_name == "category_name":
            category_match = re.search(r"(智能[\u4e00-\u9fa5A-Za-z0-9]+)", task)
            if category_match:
                parameters[parameter_name] = category_match.group(1)
...
def create_text2cypher_validation_node(
    ...
    async def validate_cypher(state: CypherState) -> Dict[str, Any]:
        ...
        """
        normalized_cypher_statement = cypher_statement.replace("$orderID", "$order_id")
        ...
        syntax_error = validate_cypher_query_syntax(
            graph=graph, cypher_statement=normalized_cypher_statement
        )
        errors.extend(syntax_error)

        # 检查Cypher查询中是否包含写操作(如CREATE、DELETE、SET等)，防止大模型意外修改数据库,
        write_errors = validate_no_writes_in_cypher_query(cypher_statement=normalized_cypher_statement)
        ...
        if llm is not None and llm_validation:
            llm_errors = await validate_cypher_query_with_llm(
                ...
                question=state.get("task", ""),
                graph=graph,
                cypher_statement=normalized_cypher_statement,
            )
            ...
            corrected_cypher_update = await correct_cypher_chain.ainvoke(
                {
                    "question": state.get("task"),
                    "errors": errors, 
                    "cypher": normalized_cypher_statement,
                    "schema": graph.schema,
                }
            )
...
def create_text2cypher_execution_node(
    ...
    async def execute_cypher(
        ...
        cypher_statement = cypher_statement.replace("$orderID", "$order_id")
        task = state.get("task", [])
        task_text = task[0] if isinstance(task, list) and task else str(task or "")
        parameter_names = set(re.findall(r"\$(\w+)", cypher_statement))
        parameters = extract_cypher_parameters(cypher_statement, task_text)
        missing_parameters = sorted(parameter_names - set(parameters.keys()))
        steps = state.get("steps", list())
        steps.append("execute_cypher")
        
        NO_CYPHER_RESULTS = [{"error": "在数据库中找不到任何相关信息。"}]
        execution_errors = list(cypher["errors"])

        if not cypher_statement:
            execution_errors.append("生成的 Cypher 为空，已跳过执行。")
            records = NO_CYPHER_RESULTS
        elif missing_parameters:
            execution_errors.append(f"缺少 Cypher 参数: {', '.join(missing_parameters)}")
            records = NO_CYPHER_RESULTS
        else:
            records = graph.query(
                query=cypher_statement,
                params=parameters,
            )
