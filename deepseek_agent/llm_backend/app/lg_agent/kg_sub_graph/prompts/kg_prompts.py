...
工具边界：
1. 店铺营业时间、店铺地址、商品基础资料、商品分类、优惠券基础信息、活动规则、使用说明、售后政策等静态或半静态知识，优先选择predefined_cypher或microsoft_graphrag_query。
2. 订单状态、用户订单列表、商品实时库存、用户商品订单、秒杀实时库存、用户是否还能购买、用户是否已经买过某张券等实时业务状态，必须选择commerce_live_query。
